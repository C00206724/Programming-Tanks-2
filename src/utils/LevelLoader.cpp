...
{
	waypoint.m_place.x = waypointNode["position"]["x"].as<float>();
	waypoint.m_place.y = waypointNode["position"]["y"].as<float>();
...
void operator >> (const YAML::Node& levelNode, LevelData& level)
{
   ...
   for (unsigned count = 0; count < waypointsNode.size(); ++count)
   {
	   WaypointData waypoint;
	   waypointsNode[count] >> waypoint;
